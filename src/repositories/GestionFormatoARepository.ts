...
export default class GestionFormatoARepository
{
    ...
    public async crearFormatoA(id:number, formato: FormatoAEntity[]): Promise<FormatoAEntity[]>{
        //TODO: Refactorizar
        const query1 = "INSERT INTO TI_A (A_OBJETIVOS, A_CON_ENTREGA, A_REALIZACION, A_RECURSOS, A_FINANCIACION, A_PER_PROGRAMA, A_RECIBIDO, A_OBSERVACIONES, A_NO_REVISION, a_interes) VALUES (?, ?, ?, ?, ?, ?, CURRENT_DATE , ? , ?, ?)";
        const query2 = "Update proceso set a_id = ? where prc_id = ?"
        const query3 = "SELECT *  FROM TI_A WHERE a_id = ?";
        console.log(id);
        
        const form:FormatoAEntity = new FormatoAEntity(formato[0].a_id,formato[0].a_objetivos,formato[0].a_con_entrega,formato[0].a_realizacion,formato[0].a_recursos,formato[0].a_financiacion,formato[0].a_per_programa,formato[0].a_revision,new Date(),formato[0].a_observaciones,formato[0].a_no_revision,'');
        const res:FormatoAEntity[] =  [];
        try{            
            const [result]:any = await db.query(query1,[form.a_objetivos,form.a_con_entrega,form.a_realizacion,form.a_recursos,form.a_financiacion,form.a_per_programa,form.a_observaciones,form.a_no_revision, form.a_interes]);
